  ...
  <v-container fluid>
    ...
    <v-dialog v-model="customConfigDialog.visible" persistent max-width="850px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="customConfigDialog.visible = false;">Cancel</v-btn>
          <v-btn color="blue darken-1" text @click="saveCustomConfiguration">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    saveCustomConfiguration() {
      try {
        let configType = protoRoot.lookupType("Configuration");
        let config = configType.fromObject(JSON.parse(this.customConfigDialog.content))
